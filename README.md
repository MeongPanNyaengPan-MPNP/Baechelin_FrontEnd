...
사용자가 보고 있는 지도 영역을 확인해 해당 위치의 위 경도를 담아 서버로 데이터를 요청해야 함. 하지만 이를 위해 사용자의 모든 액션마다 요청을 보내면 심각한 성능 저하가 우려되기 때문에 적절한 트리거를 찾아야 함
...
- 사용자의 모든 액션마다 API요청을 보내면 심각한 성능 저하가 우려됨. 이에 사용자의 유휴상태를 찾을 수 있는 이벤트가 필요함
- 이에 맵API에서 타일 로딩이 완료될 때를 알려주는 이벤트 리스너를 찾았지만 맵이 빠르게 로딩되는 환경이라면 사용자가 드래그를 하는 짧은 순간마다 API요청을 해서 성능 저하가 일어남. 또한 사용자가 원하는 위치로 가기 전에도 업장목록이 계속 갱신되기 때문에 화면이 난잡해보이고 UX적으로도 옳지 못함. 또한 추후 배슐랭 서비스에서 지도 유료API를 가져오게 된다면 금전적인 손실도 발생해 낭비되는 api 호출 횟수를 최대한 줄여야함
- 이를 위해 디바운스라는 방법을 적용함. setTimeout으로 delay를 주어 이전의 이벤트는 취소하고 가장 마지막 액션만을 실행하는 방법임. 그러나 delay를 너무 길게 가져가면 로딩 시간 지연으로 느껴질 수 있기 때문에 적절한 시간을 찾아야 했음.
